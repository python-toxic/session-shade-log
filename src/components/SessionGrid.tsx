  ...
  return <>
      <div className="bg-slate-800/30 backdrop-blur-sm rounded-xl p-6 border border-slate-700/30 hover:border-slate-600/50 transition-all animate-pop-in hover-scale">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          return <div key={session.id} className="relative group cursor-pointer animate-scale-in" style={{
            animationDelay: `${index * 0.1}s`
          }}>
                <div className="aspect-square rounded-lg border-2 border-slate-600/50 p-4 flex flex-col justify-between transition-all duration-300 hover:scale-105 hover:border-blue-500/70 backdrop-blur-sm" style={{
              backgroundColor
            }}>
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="flex justify-end gap-2 mt-1">
                      {stats.completed > 0 && <span className="text-xs text-green-400">✓{stats.completed}</span>}
                      {stats.pending > 0 && <span className="text-xs text-yellow-400">⏳{stats.pending}</span>}
                    </div>
                  </div>
                </div>

                {/* Target Progress Ring */}
                {stats.total > 0 && <div className="absolute -top-1 -right-1">
                  ...
                  transform: `rotate(${stats.total / target * 360 - 90}deg)`
                      ...
                      {stats.total >= target && <div className="absolute inset-1 bg-green-500 rounded-full animate-pulse-ring" />}
                    </div>
                  </div>}

                {/* Tooltip */}
                {stats.total > 0 && <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                    <div className="bg-gray-900/90 backdrop-blur-sm text-white text-xs rounded-lg p-3 shadow-lg min-w-48 max-w-64">
                      ...
                      <div className="space-y-1">
                        {getSessionTasks(session.id).map((task, index) => <div key={index} className="text-gray-300 flex items-center gap-2">
                            {task.pomodoroCompleted ? '✅' : '⏳'} {task.text}
                            {task.pomodoroTime && task.pomodoroTime > 0 && <span className="text-blue-400 text-xs">🍅{task.pomodoroTime}m</span>}
                          </div>)}
                      </div>
                    </div>
                  </div>}
